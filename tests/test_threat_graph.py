...
class ThreatGraphEngineTests(unittest.TestCase):
    ...
    def test_threat_graph_api_endpoint(self):
        analyst = User(username="analyst_test", email="analyst@guardly.sec", password="password", role=User.ROLE_USER)
        other_analyst = User(username="other_analyst", email="other@guardly.sec", password="password", role=User.ROLE_USER)
        db.session.add(analyst)
        db.session.add(other_analyst)
        db.session.commit()

        own_scan = EmailScan(subject="Analyst-private scan", sender="own@example.com", user_id=analyst.id)
        other_scan = EmailScan(subject="Other-analyst-private scan", sender="other@example.com", user_id=other_analyst.id)
        db.session.add_all([own_scan, other_scan])
        db.session.commit()
        ...
        labels = [node["label"] for node in json_data["nodes"]]
        self.assertTrue(any("Analyst-private scan" in label for label in labels))
        self.assertFalse(any("Other-analyst-private scan" in label for label in labels))
