...
class EndToEndPipelineIntegrationTests(unittest.TestCase):
    ...
    def test_threat_analysis_with_none_return_path_and_reply_to(self):
        """Verify that parsed email with None return_path does not raise TypeError."""
        email_obj = {
            "message_id": "test_msg_none_rp",
            "from": "user@example.com",
            "reply_to": None,
            "return_path": None,
            "subject": "Test subject with none return path",
            "text_body": "Clean test email body.",
            "html_body": "",
            "urls": [],
            "attachments": [],
        }
        engine = ThreatAnalysisEngine()
        result = engine.analyze(email_obj)
        self.assertIsNotNone(result)
        self.assertEqual(result["sender_analysis"]["return_path_domain"], "")
        self.assertEqual(result["sender_analysis"]["reply_to_domain"], "")

    def test_quishing_attachment_file_scanning_on_disk(self):
        """Verify image attachment on disk is read and evaluated for quishing in AttachmentAnalyzer."""
        img_file = os.path.join(self.temp_dir.name, "test_qr.png")
        sample_img_payload = b"\x89PNG\r\n\x1a\n\x00\x00https://malicious-qr-target.com/login\x00\x00"
        with open(img_file, "wb") as f:
            f.write(sample_img_payload)

        email_obj = {
            "message_id": "test_msg_quish",
            "from": "user@example.com",
            "subject": "Quishing scan",
            "text_body": "Check attachment",
            "attachments": [
                {
                    "filename": "test_qr.png",
                    "original_filename": "test_qr.png",
                    "mime_type": "image/png",
                    "size": len(sample_img_payload),
                    "sha256": "fakehash",
                    "storage_path": img_file,
                }
            ],
        }
        engine = ThreatAnalysisEngine()
        result = engine.analyze(email_obj)
        self.assertTrue(any("QR code embedded in image attachment" in f for f in result["findings"]))
