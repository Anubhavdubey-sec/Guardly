...
from routes.scanner import _safe_csv_cell
...
class SecurityHardeningTests(unittest.TestCase):
    ...
    def test_csv_export_cells_cannot_execute_spreadsheet_formulas(self):
        self.assertEqual(_safe_csv_cell("=HYPERLINK(\"https://evil.example\")"), "'=HYPERLINK(\"https://evil.example\")")
        self.assertEqual(_safe_csv_cell(" \t@SUM(1,1)"), "' \t@SUM(1,1)")
        self.assertEqual(_safe_csv_cell("ordinary subject"), "ordinary subject")
    ...
    def test_admin_self_lockout_prevention(self):
        ...
        # Attempt to demote sole admin to user (last admin lockout protection)
        res = client.post(
            f"/admin/users/{admin_id}/role",
            data={"csrf_token": "test_csrf_token_admin", "role": User.ROLE_USER},
