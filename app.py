...
def ensure_mysql_database():
    ...
    server_engine = create_engine(
        db.engine.url.set(database=None),
        pool_pre_ping=True,
    )
...
def create_app(test_config=None):
    ...
    if "RATELIMIT_ENABLED" not in app.config:
        app.config["RATELIMIT_ENABLED"] = not app.config.get(
            "TESTING",
            False,
        )
    ...
    if app.config["AUTO_CREATE_SCHEMA"] or app.config.get("TESTING"):
    ...
    def handle_internal_error(error):
        ...
        return (
            render_template(
                "upload.html",
                error=(
                    "An internal security exception occurred. "
...
def register_cli_commands(app):
    ...
    def seed_users_cmd():
        if app.config.get("ENV") == "production" or not app.config.get(
            "DEBUG"
        ):
            click.echo(
                "Error: 'seed-users' command is disabled in "
                "production / non-debug mode for security.",
                err=True,
            )
            return
        ...
        for username, email, role in staff_accounts:
            ...
            is_valid, errors, _ = validate_password(
                password,
                username=username,
                email=email,
            )
...
def migrate_sqlite_data(source):
    if db.engine.dialect.name != "mysql":
        raise click.UsageError(
            "Set DATABASE_URL to a MySQL database before "
    ...
    try:
                ...
                db.session.add(
                    User(
                        ...
                        email=_sqlite_value(
                            row,
                            "email",
                            f"migrated-{row['id']}@invalid.local",
                        ),
                        password=_sqlite_value(
                            row,
                            "password",
                            "!",
                        ),
        ...
        if "email_scans" in source_tables:
            for row in source_db.execute(
                "SELECT * FROM email_scans ORDER BY id"
            ):
                if db.session.get(
                    EmailScan,
                    row["id"],
                ...
                db.session.add(
                    EmailScan(
                        ...
                        sender=_sqlite_value(
                            row,
                            "sender",
                        ),
                        receiver=_sqlite_value(
                            row,
                            "receiver",
                        ),
                        subject=_sqlite_value(
                            row,
                            "subject",
                        ),
                        email_date=_sqlite_value(
                            row,
                            "email_date",
                        ),
                        reply_to=_sqlite_value(
                            row,
                            "reply_to",
                        ),
                        email_body=_sqlite_value(
                            row,
                            "email_body",
                        ),
                        risk_score=risk_score,
                        verdict=_sqlite_verdict(
                            _sqlite_value(
                                row,
                                "verdict",
                            ),
                        ...
                        attachments=_sqlite_value(
                            row,
                            "attachments",
                            "[]",
                        ),
                        headers=_sqlite_value(
                            row,
                            "headers",
                            "{}",
                        ),
                        ...
                        risk_categories=_sqlite_value(
                            row,
                            "risk_categories",
                            "[]",
                        ),
                        reputation_data=_sqlite_value(
                            row,
                            "reputation_data",
                            "{}",
                        ),
        ...
        if "system_logs" in source_tables:
            for row in source_db.execute(
                "SELECT * FROM system_logs ORDER BY id"
            ):
                if db.session.get(
                    SystemLog,
                    row["id"],
                ...
                db.session.add(
                    SystemLog(
                        id=row["id"],
                        actor_id=_sqlite_value(
                            row,
                            "actor_id",
                        ),
                        actor_name=_sqlite_value(
                            row,
                            "actor_name",
                            "System",
                        ),
                        ...
                        target_type=_sqlite_value(
                            row,
                            "target_type",
                            "system",
                        ),
                        target_id=_sqlite_value(
                            row,
                            "target_id",
                        ),
                        detail=_sqlite_value(
                            row,
                            "detail",
                            "",
                        ),
    ...
    click.echo(
        "Migration complete: "
        f"{migrated['users']} users, "
        f"{migrated['email_scans']} scans, and "
...
def create_admin(username, email):
    ...
    if user:
        ...
        click.echo(
            f"Promoted {user.email} to administrator."
        )
    ...
    if not is_valid:
        raise click.UsageError(
            f"Password rejected: {' '.join(errors)}"
        )
    ...
    click.echo(
        f"Created administrator {user.email}."
    )
...
def reset_admin_password(email):
    ...
    record_event(
        ...
        target_id=user.id,
        detail=(
            "Administrator password reset from "
            f"the local CLI for {user.username}."
        ),
        actor_name="CLI",
    )
    ...
    click.echo(
        f"Password updated for administrator {user.email}."
    )
...
@click.option(
    "--host",
    default=None,
    help="Host interface to bind the SMTP receiver.",
)
@click.option(
    "--port",
    type=int,
...
@click.option(
    ...
    default=True,
    help="Run background mail queue worker thread.",
)
def run_smtp_cmd(host, port, with_worker):
    ...
    smtp_host = host or app.config.get(
        "SMTP_HOST",
        "127.0.0.1",
    )

    smtp_port = port or app.config.get(
        "SMTP_PORT",
        2525,
    )

    storage_path = app.config.get(
        "MAIL_STORAGE_PATH"
    )

    max_size = app.config.get(
        "MAX_MESSAGE_SIZE",
        10 * 1024 * 1024,
    )
    ...
    click.echo(
        f"Starting Guardly SMTP Receiver "
        f"on {smtp_host}:{smtp_port}..."
    )

    server.start()

    click.echo(
        "SMTP Receiver & Queue Worker active. "
    ...
    try:
        ...
        click.echo(
            "Guardly SMTP Receiver and Queue Worker stopped."
        )
...
def run_mail_worker_cmd():
    ...
    click.echo(
        "Guardly Mail Queue Worker active. "
        "Press Ctrl+C to stop."
    )
...
def run_mail_relay_cmd():
    ...
    click.echo(
        "Guardly Outbound Mail Relay Worker active. "
        "Press Ctrl+C to stop."
    )
...
@click.option(
    ...
    help="Google Workspace email inbox to scan.",
)
@click.option(
    "--max-results",
    type=int,
...
def scan_gmail_inbox_cmd(email, max_results):
    ...
    click.echo(
        f"Starting Guardly Post-Delivery Scan for {email}..."
    )

    result = process_gmail_inbox_scans(
        app,
        [email],
    ...
    click.echo(
        "Post-Delivery Scan Complete! "
        f"Total Scanned: {result['total_scanned']}, "
        f"Remediated (Trashed): "
        f"{result['total_remediated']}"
    )
