...
def build_threat_graph_data(
    ...
    if user_id is not None:
        # Analysts may investigate their own scans and anonymous public submissions,
        # but must not receive another analyst's mail content or indicators.
        query = query.filter(or_(EmailScan.user_id == user_id, EmailScan.user_id.is_(None)))

    query = query.order_by(EmailScan.scan_time.desc())
    if scan_id:
    ...
    visible_nodes = nodes[:max(0, max_nodes)]
    ...
    return {
        ...
        "summary": {
            ...
            "high_risk_nodes": len([n for n in visible_nodes if n["risk"] == "High Risk"]),
        }
    }
