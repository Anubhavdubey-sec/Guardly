...
def enqueue_message(
    ...
    tenant_id = tenant_id or current_app.config.get("DEFAULT_TENANT_ID", "default")
    existing = MailQueue.query.filter_by(message_id=message_id, tenant_id=tenant_id).first()
    if existing:
...
def process_queue_job(message_id: str) -> bool:
    ...
    try:
        ...
        email_msg = EmailMessage.query.filter_by(message_id=message_id, tenant_id=tenant_id).first()
        if not email_msg:
            email_msg = EmailMessage(message_id=message_id, tenant_id=tenant_id)
        ...
        email_scan = EmailScan.query.filter_by(
        ...
        decision, final_status = enforce_mail_decision(email_msg, analysis_res, tenant_id=tenant_id)
