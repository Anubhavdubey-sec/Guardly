...
class SenderAnalyzer:
    ...
    def analyze(self, parsed_email: Dict[str, Any]) -> Dict[str, Any]:
        from_raw = parsed_email.get("from") or ""
        ...
        reply_domain = reply_to.split("@")[-1].strip("> ").lower() if (reply_to and "@" in reply_to) else ""
    ...
    def analyze(self, parsed_email: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], List[str], int]:
        ...
        for att in attachments:
            ...
            if ext in {".png", ".jpg", ".jpeg", ".gif", ".webp"} and storage_path and os.path.exists(storage_path):
                try:
                    with open(storage_path, "rb") as im_f:
                        image_bytes = im_f.read()
                    qr_res = scan_attachment_for_quishing(image_bytes, filename=filename, content_type=mime_type)
                    if qr_res.get("has_qr_code"):
                        for q_url in qr_res.get("qr_urls", []):
                            msg = f"QR code embedded in image attachment '{filename}' pointing to URL: {q_url}"
                            att_item["findings"].append(msg)
                            att_findings.append(msg)
                        total_penalty += 35
