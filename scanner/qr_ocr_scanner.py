...
def decode_qr_code_from_image(image_bytes: bytes) -> List[str]:
    ...
    # PIL Image inspection & zxing-cpp Barcode / QR Decoding
    if HAS_PIL and image_bytes:
        try:
            img = Image.open(io.BytesIO(image_bytes))

            # Genuine QR code / 2D Matrix decoding via zxing-cpp
            if HAS_ZXING:
                try:
                    ...
                    for b in barcodes:
                        if b.text:
                            text = b.text.strip()
                            if text.startswith(("http://", "https://")) and text not in found_urls:
                                found_urls.append(text)
                            else:
                                # Extract any URLs embedded in the QR payload
                                for u in URL_IN_IMAGE_REGEX.findall(text):
                                    clean_u = re.sub(r"[\x00-\x1f\s\"';,\)>]+$", "", u)
...
def scan_attachment_for_quishing(payload_bytes: Union[bytes, str], filename: str = "", content_type: str = "") -> Dict[str, Any]:
    """
    ...
    if isinstance(payload_bytes, str):
        if os.path.exists(payload_bytes) and os.path.isfile(payload_bytes):
            try:
    ...
    elif not isinstance(payload_bytes, (bytes, bytearray)):
        payload_bytes = b""
