...
class ComprehensiveQASystemTest(unittest.TestCase):
    ...
    def test_qa_password_validation_policy(self):
        """QA Test: Password Policy enforcement (12-128 chars, passphrase support)."""
        from services.password_validator import validate_password
        valid_short, _, _ = validate_password("P@1a")
        valid_too_long, _, _ = validate_password("a" * 129)
        valid_passphrase, _, _ = validate_password("quiet meadow sunrise")

        self.assertFalse(valid_short)
        self.assertFalse(valid_too_long)
        self.assertTrue(valid_passphrase)
    ...
    def test_qa_single_email_scanning_workflow(self):
        ...
        self.client.post("/login", data={"email": "admin@guardly.sec", "password": "Admin@12345"})
