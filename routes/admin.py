...
def _user_or_none(user_id, active_only=True):
...
def create_user():
    if request.method == "POST":
        ...
        user = User(
            ...
            role=role if role in User.ROLES else User.ROLE_USER,
        )
        db.session.add(user)
        db.session.flush()
        record_event("user_created", target_type="user", target_id=user.id,
                     detail=f"Created user {username} with role {user.role}.", actor=g.current_user)
        db.session.commit()
...
def delete_user(user_id):
    ...
    reassigned_cases = 0
    unassigned_cases = 0
    affected_cases = InvestigationCase.query.filter(
        or_(
    ...
    for case in affected_cases:
        ...
        if case.created_by_id == user.id:
            case.assigned_to_id = g.current_user.id
            reassigned_cases += 1
        else:
    ...
    record_event(
        ...
        target_id=user_id,
        detail=(
            f"Deactivated user {username}; reassigned {reassigned_cases} owned case(s) and "
            f"unassigned {unassigned_cases} collaborator case(s)."
        ),
        actor=g.current_user,
    )
    db.session.commit()
    flash(f"User {username} was deactivated and historical case ownership was retained.", "success")
    return redirect(url_for("admin.users"))
...
def reset_password(user_id):
    ...
    record_event("admin_reset_user_password", target_type="user", target_id=user.id,
                 detail=f"Admin reset password for {user.username}.", actor=g.current_user)
    db.session.commit()
